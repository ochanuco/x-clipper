...
- バックエンドは `server/` に配置し、`index.mjs` で Express API を提供。`uploads/` ディレクトリに画像を保存し `/uploads/...` で配信するため、ストレージ容量と公開 URL を意識する。
- 環境変数は `server/.env` で管理し `.env.example` を参考にする。`ASSET_BASE_URL` は Notion から到達可能なホストを指定する。
...
- アップロードされた画像は `server/uploads` に蓄積されるため、不要になったファイルのクリーンアップや CDN 配信など運用面も検討する。
